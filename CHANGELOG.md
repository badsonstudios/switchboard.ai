...
- Finished the job 0.4.0 started: the older HTML ways of styling text — `<font
  color size face>`, `<hr color size>`, `bgcolor` — are now removed from
  documents and replies too, so "a reply can't repaint itself in colours your
  theme didn't choose" is true without the footnote that used to follow it. Two
  more go with them, and they are not really about looks: `hidden`, which left
  text in the document and in a code block's **Copy** button while taking it off
  the screen, and `tabindex`, which let content add itself to — or jump the
  queue in — your Tab order. Nothing Markdown itself writes is affected; table
  alignment is the one legacy attribute that stays, because that is how Markdown
  aligns a column. (#466, #598)
