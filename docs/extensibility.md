...
| `mcp.write` | **add or remove** an MCP server, reset a project's approvals, and type `/mcp` into a live session (§5.17, #714). Split from `mcp.read` for the reason the whole vocabulary is split, and this is the sharpest instance of it: reading tells you which tools a session has, while writing **defines a program the CLI will launch** — and at `project` scope that definition lands in a file which is checked in, so it runs on a teammate's machine when they next clone. Nearer `update.install` than `settings.write`, and never implied by holding `mcp.read` |
...
| `diagnostics.report` | builds a zip of the logs and **posts a new issue** to the project's GitHub repo (#815). Its own capability for `push.send`'s reason, and the same shape: it writes machine-derived text out to a third-party host. That `update.check` already talks to that host is not a reason to share its word — that one reads a version number, this one creates something |
| `diagnostics.perf` | appends timings to the **local** performance capture file (#923). Its own word, and the weakest of this family on purpose: it writes durations and interaction names to one file on this machine and can reach nothing else. Deliberately NOT folded into `diagnostics.report`, because that one **sends** — and E21's whole premise is that measurement is instrumentation, not telemetry. A consumer allowed to time the UI must not thereby acquire the ability to post what it timed |
| `diagnostics.credential` | stores the **GitHub token** the above files with (#815). Split from `diagnostics.report` by `push.write`'s argument: depositing a secret in the OS credential store is strictly more power than filing a report, and a consumer that may file one must not silently acquire the ability to plant the credential it is filed with |
