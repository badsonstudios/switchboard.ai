...
| `probe-command-verbs.mjs` | **Which control verbs actually exist?** (#633 audit) All of the SDK's grepped set except `status`. ⚠️ Two lessons in one file: the #729 probes GUESSED names while the real list sat in #633's comment (that is how `mcp_authenticate` was missed for two PRs), and existence-testing `set_*` verbs with no arguments made three of them SUCCEED — they may have acted. |
| `probe-mcp-auth.mjs` | **Connector sign-in** (#734). Auth is remote-only: a stdio server is refused by type. The OAuth SUCCESS path is unmeasurable here — no connectors on this machine — so the refusal shapes are all this proves. |
| `probe-mcp-toggle.mjs` | **What do `mcp_toggle` / `mcp_reconnect` actually DO?** (#729 PR 2) Answers three: an absent `enabled` **disables the server** and says success; a toggle **persists** to `disabledMcpServers`; and `mcp_reconnect` **pulls in a server the session never loaded**. Brings its own throwaway server — Dan's are never named — and prints the config fingerprint before and after so persistence is a diff, not an opinion. |
| `probe-toggle-reconnect-interaction.mjs` | **Does `mcp_reconnect` undo a `mcp_toggle`?** (#729 PR 2 review) **Yes** — `disabled` → reconnect → `connected`. So Reconnect-all must skip servers the user turned off, or one press silently re-enables all of them. ⚠️ Its `disabledMcpServers` readout is WRONG — see the warning in the file; #724's case-variant project keys made it read the empty entry. |
