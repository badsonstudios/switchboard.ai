...
/**
 * S-07's depth, which is also the visible pane's (`TerminalPane`).
 *
 * It has to be the SAME NUMBER: the group's label promises "scrollback only",
 * and a shadow that kept more lines than the pane would answer a question the
...
const HOST_STYLE =
  'position:absolute;inset-block-start:0;inset-inline-start:-99999px;' +
...
export class TerminalShadow {
  ...
  /** Drop the terminal and its host. Idempotent. */
  dispose(): void {
    this.disposed = true;
    this.pending = null;
    try {
