...
// The model picker (#721, #746).
//
// This file owns what only a RENDERED dialog can answer: that a refusal leaves
// the list null rather than empty, that the CLI's own sentence reaches the
// screen, that select-then-OK really does send nothing until OK, and that an
// answer for the session you have LEFT never paints.
//
// The pure helpers it used to also cover — `currentIndex`, `rowSubtitle`,
// `failureText` — moved to `lib/model-choices.test.ts` with the code, when the
// footer's quick menu (#747) became the second surface rendering the same list.
